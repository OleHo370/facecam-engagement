// Mirrors server/src/types.ts to define the shared Socket.IO event contract.
...
export interface EngagementScores {
  ...
  // boundary (see engagementScorer.ts's NEUTRAL_BAND) so an ambiguous read
  // (e.g. quietly staring at the screen) doesn't get forced into "Bored".
